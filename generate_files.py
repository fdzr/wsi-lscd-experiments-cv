...
methods = ["wsbm"]
llms = ["mixtral", "llama3.1-8B"]
datasets = ["dwug_es", "dwug_en"]

path_to_folder = Path(f"./cv-experiments-lscd-all-hyperparameter")
...
for method in methods:
    ...
    for llm in llms:
        path_to_llm = path_to_method / llm
            ...
            if path_to_dataset.exists() is False:
                ...
                for fold in range(1, 6):
                    ...
                    correlation_results.to_csv(
                        path_to_fold / "training.csv", index=False
                    )
                    correlation_results.to_csv(
                        path_to_fold / "testing.csv", index=False
                    )
